...
#  10. Save output/RenalUrothelium_harmony_integrated.rds
################################################################################
...
# Impute any NAs (can occur for cells from pre-normalised datasets)
n_na <- sum(is.na(so$pct_mt))
if (n_na > 0) {
  med_val <- median(so$pct_mt, na.rm = TRUE)
  so$pct_mt[is.na(so$pct_mt)] <- med_val
  message(sprintf("  Imputed %d NAs in pct_mt with median (%.4f)",
                  n_na, med_val))
}
...
# Batch-correct on sample_id + technology (+ source when >1 level present).
...
# Annotated cell type plot (from scANVI, if available)
for (ct_col in c("cell_type_scanvi", "cell_type_original")) {
  if (ct_col %in% colnames(so@meta.data)) {
    p_ct <- DimPlot(
      so, group.by = ct_col, reduction = "umap_harmony",

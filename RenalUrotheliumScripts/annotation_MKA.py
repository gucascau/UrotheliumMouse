#!/usr/bin/env python3
"""
annotation_MKA.py — Annotate RenalUrothelium_preprocessed.h5ad using the
Mouse Kidney Atlas (MouseKidneyATLAS_MKA_updated.h5ad) reference.

Pipeline:
  1. Load MKA reference; convert Ensembl IDs → gene symbols if needed
...
Reference label: author_cell_type
  PTS2, PTS1, PTS3, DCT, Endo, PC, PTS3T2, CNT, MTAL, Unknown,
  CTAL, Glom-Endo, ICB, DTL-ATL, T lymph, ATL, LOH, ICA, Macro, Fib, ...
...
REF_PATH   = os.path.join(BASE_DIR, "RenalUrothelium",
                          "MouseKidneyATLAS_MKA_updated.h5ad")
QUERY_PATH = os.path.join(SCRIPT_DIR, "output",
                          "RenalUrothelium_preprocessed.h5ad")
OUT_DIR    = os.path.join(SCRIPT_DIR, "output", "annotation_MKA")
os.makedirs(OUT_DIR, exist_ok=True)

LABEL_KEY   = "cell_type"
MKA_LABEL   = "author_cell_type"   # fine-grained annotation in MKA
REF_NAME    = "MKA"
...
###############################################################################
# STEP 1: Load MKA reference
###############################################################################

print(f"\n{'='*60}")
print(f"  Loading MKA reference")
print(f"{'='*60}")
adata_ref = sc.read_h5ad(REF_PATH)
print(f"  {adata_ref.n_obs:,} cells × {adata_ref.n_vars:,} genes")
print(f"  {MKA_LABEL} values:\n",
      adata_ref.obs[MKA_LABEL].value_counts().to_string())

adata_ref = extract_raw_counts(adata_ref, "MKA")
adata_ref = ensembl_to_symbol(adata_ref, "MKA")
adata_ref.obs[LABEL_KEY] = adata_ref.obs[MKA_LABEL].astype(str)

# MKA contains "Unknown" cells — keep them but treat as unlabeled
labeled_mask = adata_ref.obs[LABEL_KEY] != "Unknown"
n_labeled    = labeled_mask.sum()
print(f"  Labeled cells (excl. 'Unknown'): {n_labeled:,} / {adata_ref.n_obs:,}")
if n_labeled == 0:
    raise ValueError(f"Column '{MKA_LABEL}' has no valid labels in reference.")
...
print(f"\n  Shared genes (MKA ∩ query): {len(shared):,}")
if len(shared) < 200:
...
print(f"\n{'='*60}")
print("  Training scVI on MKA reference")
print(f"{'='*60}")
...
sc.pl.umap(ref_sub, color=LABEL_KEY,
           title=f"{REF_NAME} reference — {MKA_LABEL}", ax=ax, show=False)
plt.tight_layout()
plt.savefig(os.path.join(OUT_DIR, f"{REF_NAME}_scVI_UMAP.pdf"), bbox_inches="tight")
plt.close()
print("  Saved: MKA_scVI_UMAP.pdf")
...
plt.savefig(os.path.join(OUT_DIR, f"{REF_NAME}_combined_scANVI_UMAP.pdf"),
            bbox_inches="tight")
plt.close()
print("  Saved: MKA_combined_scANVI_UMAP.pdf")
